...
const renderData = (json) => {
    ...
    json.forEach(function(party) {
        partiesDOM += `<li class="party" data-borough="${party.borough}">${formatDate(party.created_date)}: (${party.borough}) ${party.location_type}</li>`;
    });
...
const filterDates = () => {
    ...
    const partiesList = document.querySelectorAll('.party');

    // find total number of records in dataset
    const numRecords = partiesList.length;
    
    // determine start & end indices
    const idxStart = Math.floor((dateRange.start / 100) * numRecords);
    const idxEnd =  Math.ceil((dateRange.end / 100) * numRecords);

    // console.log("start idx: ", idxStart);
    // console.log("end idx: ", idxEnd);

    partiesList.forEach((party, idx) => {
...
const calcBoroughScales = () => {
    const partiesList = document.querySelectorAll('.party');

    // find total number of records in dataset
    const numRecords = partiesList.length;

    // calculate totals for each borough
    partiesList.forEach((party) => {
        if (party.dataset.borough == "MANHATTAN") {
            totalManhattan += 1;
        } else if (party.dataset.borough == "BROOKLYN") {
            totalBrooklyn += 1;
        } else if (party.dataset.borough == "QUEENS") {
            totalQueens += 1;
        } else if (party.dataset.borough == "BRONX") {
            totalTheBronx += 1;
        } else if (party.dataset.borough == "STATEN ISLAND") {
            totalStatenIsland += 1;
        }
    });

    // console.log("totalManhattan: ", totalManhattan);
    // console.log("totalBrooklyn: ", totalBrooklyn);
    // console.log("totalQueens: ", totalQueens);
    // console.log("totalTheBronx: ", totalTheBronx);
    // console.log("totalStatenIsland: ", totalStatenIsland);

    const percentageManhattan = totalManhattan / numRecords;
    const percentageBrooklyn = totalBrooklyn / numRecords;
    const percentageQueens = totalQueens / numRecords;
    const percentageTheBronx = totalTheBronx / numRecords;
    const percentageStatenIsland = totalStatenIsland / numRecords;

    // calculate % for each borough
    partiesList.forEach((party) => {
        if (party.dataset.borough == "MANHATTAN") {
            party.style.setProperty("--scale", percentageManhattan);
        } else if (party.dataset.borough == "BROOKLYN") {
            party.style.setProperty("--scale", percentageBrooklyn);
        } else if (party.dataset.borough == "QUEENS") {
            party.style.setProperty("--scale", percentageQueens);
        } else if (party.dataset.borough == "BRONX") {
            party.style.setProperty("--scale", percentageTheBronx);
        } else if (party.dataset.borough == "STATEN ISLAND") {
            party.style.setProperty("--scale", percentageStatenIsland);
        }
    });
